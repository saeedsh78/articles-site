...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if author_email:
            mail_subject = "دیدگاه جدید"
            message = f"دیدگاه جدید برای مقاله <<{article}>> که شما نویسنده ان هستید ارسال شده:\n{current_site}{reverse('blog:detail_article', kwargs={'slug': article.slug})}"
            email = EmailMessage(
                mail_subject, message, to=[author_email]
            )
            email.send()

        if user_email:
            mail_subject = "دیدگاه شما ثبت شد"
            message = f"دیدگاه شما برای مقاله <<{article}>> ثبت شد:\n{current_site}{reverse('blog:detail_article', kwargs={'slug': article.slug})}"
            email = EmailMessage(
        ...
        if parent_email:
            ...
            message = f"پاسخی به دیدگاه شما در مقاله  <<{article}>> ثبت شد:\n{current_site}{reverse('blog:detail_article', kwargs={'slug': article.slug})}"
            email = EmailMessage(
                mail_subject, message, to=[parent_email]
            )
            email.send()
